...
private:
   ...
   std::unordered_map<std::uint32_t, std::pair<Com_ptr<IDirect3DVertexShader9>, Com_ptr<IDirect3DPixelShader9>>> _variation_cache;
...
public:
   ...
   {
      if (auto group = _rendertypes.find(rendertype); group != _rendertypes.cend()) {
         return &group->second;
      }
